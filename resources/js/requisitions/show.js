...
function fillModalFields(row) {
    ...
    if (row["starting_date"] && row["ending_date"]) {
        ...
        startingDate = startingDate.toLocaleDateString("es-MX", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
        });
        endingDate = endingDate.toLocaleDateString("es-MX", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
        });
...
function appendEvidences(response) {
    ...
    for (let i = 0; i < paths.length; i++) {
        ...
        evidenceNode.addClass(
            "d-flex justify-content-center align-items-center border overflow-hidden",
        );
