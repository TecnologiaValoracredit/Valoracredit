<x-base-layout :scrollspy="false">
    ...
    <div class="row layout-top-spacing">
        ...
        <input type="hidden" name="user_id" id="user_id" value="{{$s_manager->user->id}}">
        <input type="hidden" name="manager_id" id="manager_id" value="{{$s_manager->id}}">

        <!-- CONTENT HERE -->
        <div class="card">
            <div class="card-body">
                <h5 class="card-title">Modificar gerente</h5>
                <form class="row g-3 needs-validation" novalidate method="POST" action="{{ route('s_managers.update', $s_manager->id) }}">
                    @csrf
                    @method("PUT")
                    <div class="d-flex justify-content-center">
                        <div class="w-100">
                            @include("s_managers.fields")
                                ...
                                <div class="d-flex mt-2 mb-2">
                                    <div>
                                        @include("components.custom.forms.input-inline-select", [
                                            ...
                                            "value" => isset($s_manager) ? $s_manager->user->institution_id :  old("institution_id"),
                                            "label" => "Institución",
                                    ...
                                    <div>
                                        <a class="btn btn-success" onclick="addInstitutionToManager()">+</a>
                                    </div>
                                </div>
                                {!! $institutionDT["view"] !!}
                            @endif
