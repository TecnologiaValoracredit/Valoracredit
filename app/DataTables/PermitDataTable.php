...
class PermitDataTable extends DataTable
{
    ...
    public function getActions($row){
        ...
        $exportChecks = $row->permit_status_name == "Aprobado";
        ...
        //Solo se puede eliminar SI tiene el permiso y SI es el dueño del permiso Y si no ha sido enviado
        if ((auth()->user()->hasPermissions("permits.destroy") && auth()->id() == $row->user_id) && (in_array($row->permit_status_name, ["Creado"]))) {
            $result .= '
        ...
        if (auth()->user()->hasPermissions('vacations.exportPdf') && $exportChecks){
            $result .= '
                <a href="' . route('permits.exportPdf', $row->id) . '" target="_blank" class="btn btn-outline-success btn-icon p-auto">
