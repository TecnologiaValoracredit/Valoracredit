...
class FClasificationDataTable extends DataTable
{
    /**
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("f_clasifications.edit")) {
            $result .= '
                <a title="Editar" href='.route("f_clasifications.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("f_clasifications.destroy")) {
            $result .= '
     ...
     *
     * @param \App\Models\FClasification $model
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("f_clasifications.edit") ||
            auth()->user()->hasPermissions("f_clasifications.create") ||
            auth()->user()->hasPermissions("f_clasifications.destroy")) {
