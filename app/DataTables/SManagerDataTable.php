...
class SManagerDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->setRowId('id')
        ->editColumn('created_at', function(SManager $role) {
            return date("d/m/Y H:i", strtotime($role->created_at));
        })
        ->editColumn('updated_at', function(SManager $role) {
            return date("d/m/Y H:i", strtotime($role->updated_at));
        })
        ->editColumn('is_active', function(SManager $role) {
            if ($role->is_active) {
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("s_managers.edit")) {
            $result .= '
                <a title="Editar" href='.route("s_managers.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("s_managers.destroy")) {
            $result .= '
     ...
     *
     * @param \App\Models\SManager $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(SManager $model): QueryBuilder
    {
        return $model->select(
            's_managers.*',
        ...
        ->leftJoin('users','s_managers.user_id','=','users.id')
        ->leftJoin('s_branches','s_managers.s_branch_id','=','s_branches.id')
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("s_managers.edit") ||
            auth()->user()->hasPermissions("s_managers.create") ||
            auth()->user()->hasPermissions("s_managers.destroy")) {
    ...
    {
        return 's_managers_' . date('YmdHis');
    }
}
