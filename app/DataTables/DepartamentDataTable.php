...
class DepartamentDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->setRowId('id')
        ->editColumn('created_at', function(Departament $role) {
            return date("d/m/Y H:i", strtotime($role->created_at));
        })
        ->editColumn('updated_at', function(Departament $role) {
            return date("d/m/Y H:i", strtotime($role->updated_at));
        })
        ->editColumn('is_active', function(Departament $role) {
            if ($role->is_active) {
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("departaments.edit")) {
            $result .= '
                <a title="Editar" href='.route("departaments.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("departaments.destroy")) {
            $result .= '
     ...
     */
    public function query(Departament $model): QueryBuilder
    {
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("departaments.edit") ||
            auth()->user()->hasPermissions("departaments.create") ||
            auth()->user()->hasPermissions("departaments.destroy")) {
    ...
    {
        return 'Departaments_' . date('YmdHis');
    }
}
