...
class BranchDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->setRowId('id')
        ->editColumn('created_at', function(Branch $role) {
            return date("d/m/Y H:i", strtotime($role->created_at));
        })
        ->editColumn('updated_at', function(Branch $role) {
            return date("d/m/Y H:i", strtotime($role->updated_at));
        })
        ->editColumn('is_active', function(Branch $role) {
            if ($role->is_active) {
    ...
    public function getActions($row){
        $result = null;
        if (auth()->user()->hasPermissions("branches.edit")) {
            $result .= '
                <a title="Editar" href='.route("branches.edit", $row->id).' class="btn btn-outline-secondary btn-icon ps-2 px-1">
        ...
        if (auth()->user()->hasPermissions("branches.destroy")) {
            $result .= '
    ...
    public function getColumns(): array
    {
        ...
        if (auth()->user()->hasPermissions("branches.edit") ||
            auth()->user()->hasPermissions("branches.create") ||
            auth()->user()->hasPermissions("branches.destroy")) {
